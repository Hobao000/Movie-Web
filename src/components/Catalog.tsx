...
import { BANNED_REGEX } from "@/utils/contentFilter";
...
export default function MediaCatalog({ searchParams, category }: CatalogProps) {
  ...
  const {
    ...
    queryKey: ["media-catalog", category, currentQuery, currentType], 
    queryFn: ({ pageParam = 1 }) => {
      if (currentQuery) {
        return movieApi.search(category, currentQuery, pageParam as number);
      }
      if (currentType === "top_rated") {
        return movieApi.getTopRated(category, pageParam as number);
      }
      return movieApi.getPopular(category, pageParam as number);
    },
    initialPageParam: 1,
  ...
  const getPageTitle = () => {
    ...
    const mediaName = category === "movie" ? "Movies" : "TV Series";
    return currentType === "top_rated" ? `Top Rated ${mediaName}` : `Popular ${mediaName}`;
  };
      ...
      <div>
        ...
        <div className="bg-black-main px-4 md:px-8 py-8 xl:p-16 relative z-20">
          <div className="max-w-screen-2xl mx-auto">
            ...
            <form onSubmit={handleSearch} className="flex items-center relative rounded-full bg-black/40 backdrop-blur-md w-full max-w-full md:w-fit lg:w-fit border border-white/10 p-1 mb-12 shadow-xl">
              <input
                type="text"
              ...
              />
              <button type="submit" className="btn-sm btn-primary py-2 px-5 md:px-10 rounded-full shadow-lg hover:shadow-red-main/50 transition-all font-bold text-sm md:text-base whitespace-nowrap">
                Search
              </button>
            </form>
            ...
            {!isLoading && movies.length === 0 && (
              <div className="w-full py-20 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-gray-400 max-w-md">
                  Rất tiếc, BaoMovies không tìm thấy phim nào khớp với từ khóa &quot;<span className="text-red-main font-semibold">{currentQuery}</span>&quot;. Vui lòng thử lại.
                </p>
              </div>
            )}
